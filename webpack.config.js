...
if (debug) {
    ...
    require('child_process').exec(`start ${http}://${getLocalIP()}:${port}/tag_demo/`);
    require('child_process').exec(`open ${http}://${getLocalIP()}:${port}/tag_demo/`);
} else {
...
function getLocalIP() {
    ...
    for (const devname in nets) {
        if (devname.indexOf('Pseudo') >= 0) continue;

        const device = nets[devname];
        for (const i in device) {
            const network = device[i];
            const head = network.address.split('.')[0];
